...
public class Panel extends JPanel implements Runnable {
    ...
    private int enemySpawnPoint = rnd.nextInt(7) + 1;
    private int strikeSpawnPoint = rnd.nextInt(10);
    private int strikeSpawn;
    public int killCount = 0;
    private Thread thread;
    private final KeyHandler key = new KeyHandler();
    private final SoundManager soundM = new SoundManager();
    private final Player player = new Player(this, key, soundM);
    private final TileManager tileM = new TileManager(this);

    //Arraylist for updating and drawing entities
    public final ArrayList<Strike> strikeList = new ArrayList<>();
    public final ArrayList<Projectile> projectileList = new ArrayList<>();
    ...
    public void addEnemy() {
        if (player.active && enemyList.size() < 6) {
            enemyList.add(new Enemy(rnd.nextInt(960), rnd.nextInt(250) + 400, player.x, player, this, soundM));
        }
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        g2.drawString("Kill Counter: " + killCount, 5, 40);
